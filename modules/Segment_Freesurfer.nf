process Segment_Freesurfer {
    ...
    scil_volume_math.py convert $aparc aparc+aseg_int16.nii.gz --data_type int16 -f
    scil_volume_math.py convert $wmparc wmparc_int16.nii.gz --data_type int16 -f
    ...
    scil_volume_math.py addition ${sid}__mask_wm_bin.nii.gz\
                                ${sid}__mask_nuclei_bin.nii.gz\
                                ${sid}__mask_wm.nii.gz --data_type int16

    scil_volume_math.py convert ${sid}__mask_wm.nii.gz ${sid}__mask_wm.nii.gz --data_type uint8 -f
    scil_volume_math.py convert ${sid}__mask_gm.nii.gz ${sid}__mask_gm.nii.gz --data_type uint8 -f
    scil_volume_math.py convert ${sid}__mask_csf.nii.gz ${sid}__mask_csf.nii.gz --data_type uint8 -f
    """
}